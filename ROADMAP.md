...
3.  **참여자 상태 실시간 관리**: 
    - WebSocket `$connect`/`$disconnect` 이벤트를 활용하여 사용자의 온라인 상태를 실시간으로 브로드캐스트하고, 참여자 목록 UI를 자동 갱신할 것.
...
### [우선순위: 공통] UI/UX 고도화 (Polishing Phase)
1.  **전역 Toast 알림 도입**:
    - 로그인 성공/실패, 방 생성 완료, 파일 업로드 결과 등 주요 액션에 대한 시각적 피드백 제공.
    - `useToast` 커스텀 훅을 통해 컴포넌트 어디서든 호출 가능한 구조 설계.
...
## 4. 개발 시 유의사항
- **필드명 준수**: 프론트엔드와 백엔드 간 필드명 불일치(예: `title` vs `roomName`)가 발생하지 않도록 `seedData.js`의 규격을 엄격히 따를 것.
